...
// Soal No 1
console.log(
  "soal No 1. Buat array baru berdasarkan script pada link diatas dengan data data berikut -idTransaction -storeLocation -userEmail -purchaseMethod -item yang dibelanjakan {name, quantity, price}");
console.log("-----------------------");
...
newData.forEach((sale) => {
  console.log(`Transaction ID: ${sale.idTransaction}`);
  console.log(`Store Location: ${sale.storeLocation}`);
  console.log(`User Email: ${sale.userEmail}`);
  console.log(`Purchase Method: ${sale.purchaseMethod}`);
...
// Soal No 2
console.log(
  "Soal No 2. Kelompokkan data data transaksi berdasarkan script pada link diatas dalam bentuk array pada sebuah object, berdasarkan lokasi pembelian"
);
const groupedData = data.reduce((acc, curr) => {
  const location = curr.storeLocation;
  ...
  const transactions = acc[location];

  if (!transactions[transactionId]) {
    transactions[transactionId] = {
      location: curr.storeLocation,
      saleDate: new Date(
        parseInt(curr.saleDate.$date.$numberLong)
      ).toLocaleDateString(),
...
// soal no 3
console.log(
  "Soal No 3. Buat array berdasarkan script pada link diatas yang menyimpan keuntungan yang didapat dari setiap transaksi"
);
const profits = [];
...
//soal No 4
console.log(
  "Soal No 4. Buat array berdasarkan script pada link diatas yang menyimpan jumlah barang yang dibeli pada setiap transaksi"
);
const totalItems = [];
...
data.forEach((transaction) => {
  let itemsCount = 0;
  transaction.items.forEach((item) => {
    itemsCount += parseInt(item.quantity.$numberInt);
  });
  totalItems.push(itemsCount);
});
...
//Soal No 5
console.log(
  "Soal No 5. Buat array berdasarkan script pada link diatas yang menyimpan rating setiap toko dari satisfaction customer mereka"
);
const storeRatings = {};
...
data.forEach((transaction) => {
  ...
  if (!storeRatings[location]) {
    storeRatings[location] = {
      count: 1,
      rating: transaction.customer.satisfaction.$numberInt,
    };
  } else {
    storeRatings[location].count += 1;
    storeRatings[location].rating +=
      transaction.customer.satisfaction.$numberInt;
  }
});

const storeAvgRatings = {};

for (const location in storeRatings) {
  const ratingSum = storeRatings[location].rating;
  const ratingCount = storeRatings[location].count;
  storeAvgRatings[location] = ratingSum / ratingCount;
}

console.log(storeAvgRatings);

//Soal No 6
console.log(
  "Soal No 6. Buat array berdasarkan script pada link diatas yang menyimpan  keuntungan yang didapat dari setiap lokasi toko"
);
const locationProfits = {};
...
data.forEach((transaction) => {
  ...
  if (locationProfits[location]) {
    locationProfits[location] += totalProfit;
  } else {
    locationProfits[location] = totalProfit;
  }
});

console.log(locationProfits);
